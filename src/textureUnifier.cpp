		...
		if(&in == &inputs[triggerTextureIndex].get()){
			...
			for(int i = 0; i < inputs.size(); i++){
				...
				if(t != nullptr){
					if(customPositions[i].get() == "0,0"){
						...
						fboRect.growToInclude(ofRectangle(ofToInt(positionInfo[0]), ofToInt(positionInfo[1]),
														  t.get()->getWidth(), t.get()->getHeight()));
					}
				}
			}
			if(!lastSpacing){
			...
			if(outputFbo.getHeight() != fboRect.getHeight() || outputFbo.getWidth() != fboRect.getWidth() || !outputFbo.isAllocated()){
				if(fboRect.getHeight() != 0 && fboRect.getWidth() != 0){
					...
					settings.internalformat = GL_RGB32F;   // float buffer helps avoid clamp when adding
			...
			// --- Additive blend for overlapping inputs ---
			ofPushStyle();
			...
			for(int i = 0; i < inputs.size(); i++){
				auto &t = inputs[i];
				if(t != nullptr){
					// Use alpha to scale intensity for additive blend
					ofSetColor(255, 255, 255, (int)ofClamp(255.f * opacities[i], 0.f, 255.f));
					...
					if(customPositions[i].get() == "0,0"){
						...
						vector<string> positionInfo = ofSplitString(customPositions[i], ",");
						int px = ofToInt(positionInfo[0]);
						int py = ofToInt(positionInfo[1]);
						t.get()->draw(px, py);
						currentLine = max(currentLine, py + (int)t.get()->getHeight()) + spacing;
					}
				}
			}

			// restore style (disables blending back to previous mode)
			ofPopStyle();
			// ---------------------------------------------
